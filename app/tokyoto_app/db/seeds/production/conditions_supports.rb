ConditionsSupport.create!(
  [
    {
      id: 1,
      condition_id: 1,
      support_id: 7,
      city_id: 7,
      payment: 100000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/iwaikinkofu.html",
      payment_limit: 0,
      payment_frequency: 5,
      payment_month: "記載なし",
      transfer_destination: "記載なし",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-生活保護-第一子-ある-ある
    {
      id: 2,
      condition_id: 2,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-生活保護-第一子-ある-ない
    {
      id: 3,
      condition_id: 3,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-生活保護-第一子-ない-ある
    {
      id: 4,
      condition_id: 4,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-生活保護-第一子-ない-ない
    {
      id: 5,
      condition_id: 5,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-生活保護-第二子-ある-ある
    {
      id: 6,
      condition_id: 6,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-生活保護-第二子-ある-ない
    {
      id: 7,
      condition_id: 7,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-生活保護-第二子-ない-ある
    {
      id: 8,
      condition_id: 8,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-生活保護-第二子-ない-ない
    {
      id: 9,
      condition_id: 9,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
##私立幼稚園等保護者補助金-生活保護-第三子-ある-ある
    {
      id: 10,
      condition_id: 10,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-生活保護-第三子-ある-ない
    {
      id: 11,
      condition_id: 11,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-生活保護-第三子-ない-ある
    {
      id: 12,
      condition_id: 12,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-生活保護-第三子-ない-ない
    {
      id: 13,
      condition_id: 13,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第一子-ある-ある
    {
      id: 14,
      condition_id: 14,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第一子-ある-ない
    {
      id: 15,
      condition_id: 15,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第一子-ない-ある
    {
      id: 16,
      condition_id: 16,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第一子-ない-ない
    {
      id: 17,
      condition_id: 17,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第二子-ある-ある
    {
      id: 18,
      condition_id: 18,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第二子-ある-ない
    {
      id: 19,
      condition_id: 19,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第二子-ない-ある
    {
      id: 20,
      condition_id: 20,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第二子-ない-ない
    {
      id: 21,
      condition_id: 21,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第三子-ある-ある
    {
      id: 22,
      condition_id: 22,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第三子-ある-ない
    {
      id: 23,
      condition_id: 23,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第三子-ない-ある
    {
      id: 24,
      condition_id: 24,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第三子-ない-ない
    {
      id: 25,
      condition_id: 25,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-360万未満-第一子-ある-ある
    {
      id: 26,
      condition_id: 26,
      support_id: 20,
      city_id: 7,
      payment: 141600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-360万未満-第一子-ある-ない
    {
      id: 27,
      condition_id: 27,
      support_id: 20,
      city_id: 7,
      payment: 141600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-360万未満-第一子-ない-ある
    {
      id: 28,
      condition_id: 28,
      support_id: 20,
      city_id: 7,
      payment: 141600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-360万未満-第一子-ない-ない
    {
      id: 29,
      condition_id: 29,
      support_id: 20,
      city_id: 7,
      payment: 141600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-360万未満-第二子-ある-ある
    {
      id: 30,
      condition_id: 30,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-360万未満-第二子-ある-ない
    {
      id: 31,
      condition_id: 31,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-360万未満-第二子-ない-ある
    {
      id: 32,
      condition_id: 32,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-360万未満-第二子-ない-ない
    {
      id: 33,
      condition_id: 33,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-360万未満-第三子-ある-ある
    {
      id: 34,
      condition_id: 34,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-360万未満-第三子-ある-ない
    {
      id: 35,
      condition_id: 35,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409",
    },
    ##私立幼稚園等保護者補助金-360万未満-第三子-ない-ある
    {
      id: 36,
      condition_id: 36,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-360万未満-第三子-ない-ない
    {
      id: 37,
      condition_id: 37,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409",
    },
    ##私立幼稚園等保護者補助金-680万未満-第一子-ある-ある
    {
      id: 38,
      condition_id: 38,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-680万未満-第一子-ある-ない
    {
      id: 39,
      condition_id: 39,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-680万未満-第一子-ない-ある
    {
      id: 40,
      condition_id: 40,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-680万未満-第一子-ない-ない
    {
      id: 41,
      condition_id: 41,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-680万未満-第二子-ある-ある
    {
      id: 42,
      condition_id: 42,
      support_id: 20,
      city_id: 7,
      payment: 48000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-680万未満-第二子-ある-ない
    {
      id: 43,
      condition_id: 43,
      support_id: 20,
      city_id: 7,
      payment: 48000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-680万未満-第二子-ない-ある
    {
      id: 44,
      condition_id: 44,
      support_id: 20,
      city_id: 7,
      payment: 48000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-680万未満-第二子-ない-ない
    {
      id: 45,
      condition_id: 45,
      support_id: 20,
      city_id: 7,
      payment: 48000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-680万未満-第三子-ある-ある
    {
      id: 46,
      condition_id: 46,
      support_id: 20,
      city_id: 7,
      payment: 186000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-680万未満-第三子-ある-ない
    {
      id: 47,
      condition_id: 47,
      support_id: 20,
      city_id: 7,
      payment: 186000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-680万未満-第三子-ない-ある
    {
      id: 48,
      condition_id: 48,
      support_id: 20,
      city_id: 7,
      payment: 186000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-680万未満-第三子-ない-ない
    {
      id: 49,
      condition_id: 49,
      support_id: 20,
      city_id: 7,
      payment: 186000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第一子-ある-ある
    {
      id: 50,
      condition_id: 50,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第一子-ある-ない
    {
      id: 51,
      condition_id: 51,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第一子-ない-ある
    {
      id: 52,
      condition_id: 52,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第一子-ない-ない
    {
      id: 53,
      condition_id: 53,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第二子-ある-ある
    {
      id: 54,
      condition_id: 54,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第二子-ある-ない
    {
      id: 55,
      condition_id: 55,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第二子-ない-ある
    {
      id: 56,
      condition_id: 56,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第二子-ない-ない
    {
      id: 57,
      condition_id: 57,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第三子-ある-ある
    {
      id: 58,
      condition_id: 58,
      support_id: 20,
      city_id: 7,
      payment: 178800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第三子-ある-ない
    {
      id: 59,
      condition_id: 59,
      support_id: 20,
      city_id: 7,
      payment: 178800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第三子-ない-ある
    {
      id: 60,
      condition_id: 60,
      support_id: 20,
      city_id: 7,
      payment: 178800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万未満-第三子-ない-ない
    {
      id: 61,
      condition_id: 61,
      support_id: 20,
      city_id: 7,
      payment: 178800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第一子-ある-ある
    {
      id: 62,
      condition_id: 62,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第一子-ある-ない
    {
      id: 63,
      condition_id: 63,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第一子-ない-ある
    {
      id: 64,
      condition_id: 64,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第一子-ない-ない
    {
      id: 65,
      condition_id: 65,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第二子-ある-ある
    {
      id: 66,
      condition_id: 66,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第二子-ある-ない
    {
      id: 67,
      condition_id: 67,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第二子-ない-ある
    {
      id: 68,
      condition_id: 68,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第二子-ない-ない
    {
      id: 69,
      condition_id: 69,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第三子-ある-ある
    {
      id: 70,
      condition_id: 70,
      support_id: 20,
      city_id: 7,
      payment: 118800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第三子-ある-ない
    {
      id: 71,
      condition_id: 71,
      support_id: 20,
      city_id: 7,
      payment: 118800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第三子-ない-ある
    {
      id: 72,
      condition_id: 72,
      support_id: 20,
      city_id: 7,
      payment: 118800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    ##私立幼稚園等保護者補助金-730万以上-第三子-ない-ない
    {
      id: 73,
      condition_id: 73,
      support_id: 20,
      city_id: 7,
      payment: 118800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "庶務課庶務係 私立幼稚園担当,電話：03-5246-1236,ファクス：03-5246-1409"
    },
    {
      id: 114,
      condition_id: 100,
      support_id: 9,
      city_id: 7,
      payment: 162000,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidoikuseiteate.html",
      payment_limit: 0,
      payment_frequency: 4,
      payment_month: "2月(10~1月分),6月(2~5月分),10月(6~9月分)",
      transfer_destination: "",
      user_application_limit: "",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "6月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 115,
      condition_id: 101,
      support_id: 9,
      city_id: 7,
      payment: 162000,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidoikuseiteate.html",
      payment_limit: 0,
      payment_frequency: 4,
      payment_month: "2月(10~1月分),6月(2~5月分),10月(6~9月分)",
      transfer_destination: "",
      user_application_limit: "",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "6月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 116,
      condition_id: 102,
      support_id: 9,
      city_id: 7,
      payment: 162000,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidoikuseiteate.html",
      payment_limit: 0,
      payment_frequency: 4,
      payment_month: "2月(10~1月分),6月(2~5月分),10月(6~9月分)",
      transfer_destination: "",
      user_application_limit: "",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "6月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 117,
      condition_id: 103,
      support_id: 9,
      city_id: 7,
      payment: 162000,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidoikuseiteate.html",
      payment_limit: 0,
      payment_frequency: 4,
      payment_month: "2月(10~1月分),6月(2~5月分),10月(6~9月分)",
      transfer_destination: "",
      user_application_limit: "",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "6月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 118,
      condition_id: 104,
      support_id: 9,
      city_id: 7,
      payment: 162000,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidoikuseiteate.html",
      payment_limit: 0,
      payment_frequency: 4,
      payment_month: "2月(10~1月分),6月(2~5月分),10月(6~9月分)",
      transfer_destination: "",
      user_application_limit: "",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "6月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 119,
      condition_id: 105,
      support_id: 9,
      city_id: 7,
      payment: 162000,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidoikuseiteate.html",
      payment_limit: 0,
      payment_frequency: 4,
      payment_month: "2月(10~1月分),6月(2~5月分),10月(6~9月分)",
      transfer_destination: "",
      user_application_limit: "",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "6月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 120,
      condition_id: 106,
      support_id: 9,
      city_id: 7,
      payment: 162000,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidoikuseiteate.html",
      payment_limit: 0,
      payment_frequency: 4,
      payment_month: "2月(10~1月分),6月(2~5月分),10月(6~9月分)",
      transfer_destination: "",
      user_application_limit: "",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "6月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 100,
      condition_id: 100,
      support_id: 8,
      city_id: 7,
      payment: 529680,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
      payment_limit: 0,
      payment_frequency: 2,
      payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "8月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 101,
      condition_id: 101,
      support_id: 8,
      city_id: 7,
      payment: 529680,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
      payment_limit: 0,
      payment_frequency: 2,
      payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "8月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 102,
      condition_id: 102,
      support_id: 8,
      city_id: 7,
      payment: 125040,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
      payment_limit: 0,
      payment_frequency: 2,
      payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "8月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 103,
      condition_id: 103,
      support_id: 8,
      city_id: 7,
      payment: 529680,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
      payment_limit: 0,
      payment_frequency: 2,
      payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "8月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 104,
      condition_id: 104,
      support_id: 8,
      city_id: 7,
      payment: 125040,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
      payment_limit: 0,
      payment_frequency: 2,
      payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "8月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 105,
      condition_id: 105,
      support_id: 8,
      city_id: 7,
      payment: 75000,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
      payment_limit: 0,
      payment_frequency: 2,
      payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "8月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    {
      id: 106,
      condition_id: 106,
      support_id: 8,
      city_id: 7,
      payment: 529680,
      age_id: 3,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
      payment_limit: 0,
      payment_frequency: 2,
      payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
      user_renewal_month: "8月",
      contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    },
    # {
    #   id: 107,
    #   condition_id: 100,
    #   support_id: 8,
    #   city_id: 7,
    #   payment: 0,
    #   age_id: 3,
    #   url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
    #   payment_limit: 0,
    #   payment_frequency: 2,
    #   payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
    #   transfer_destination: "指定した口座",
    #   user_application_limit: "記載なし",
    #   user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
    #   user_renewal_month: "8月",
    #   contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    # },
    # {
    #   id: 108,
    #   condition_id: 101,
    #   support_id: 8,
    #   city_id: 7,
    #   payment: 0,
    #   age_id: 3,
    #   url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
    #   payment_limit: 0,
    #   payment_frequency: 2,
    #   payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
    #   transfer_destination: "指定した口座",
    #   user_application_limit: "記載なし",
    #   user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
    #   user_renewal_month: "8月",
    #   contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    # },
    # {
    #   id: 109,
    #   condition_id: 102,
    #   support_id: 8,
    #   city_id: 7,
    #   payment: 0,
    #   age_id: 3,
    #   url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
    #   payment_limit: 0,
    #   payment_frequency: 2,
    #   payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
    #   transfer_destination: "指定した口座",
    #   user_application_limit: "記載なし",
    #   user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
    #   user_renewal_month: "8月",
    #   contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    # },
    # {
    #   id: 110,
    #   condition_id: 103,
    #   support_id: 8,
    #   city_id: 7,
    #   payment: 0,
    #   age_id: 3,
    #   url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
    #   payment_limit: 0,
    #   payment_frequency: 2,
    #   payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
    #   transfer_destination: "指定した口座",
    #   user_application_limit: "記載なし",
    #   user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
    #   user_renewal_month: "8月",
    #   contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    # },
    # {
    #   id: 111,
    #   condition_id: 104,
    #   support_id: 8,
    #   city_id: 7,
    #   payment: 0,
    #   age_id: 3,
    #   url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
    #   payment_limit: 0,
    #   payment_frequency: 2,
    #   payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
    #   transfer_destination: "指定した口座",
    #   user_application_limit: "記載なし",
    #   user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
    #   user_renewal_month: "8月",
    #   contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    # },
    # {
    #   id: 112,
    #   condition_id: 105,
    #   support_id: 8,
    #   city_id: 7,
    #   payment: 0,
    #   age_id: 3,
    #   url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
    #   payment_limit: 0,
    #   payment_frequency: 2,
    #   payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
    #   transfer_destination: "指定した口座",
    #   user_application_limit: "記載なし",
    #   user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
    #   user_renewal_month: "8月",
    #   contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    # },
    # {
    #   id: 113,
    #   condition_id: 106,
    #   support_id: 8,
    #   city_id: 7,
    #   payment: 0,
    #   age_id: 3,
    #   url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
    #   payment_limit: 0,
    #   payment_frequency: 2,
    #   payment_month: "1月(11~12月分),3月(1~2月分),5月(3~4月分),7月(5~6月分),9月(7~8月分),11月(9~10月分)",
    #   transfer_destination: "指定した口座",
    #   user_application_limit: "記載なし",
    #   user_renewal_method: "受給資格の確認のため、毎年8月に現況届の提出が必要です。",
    #   user_renewal_month: "8月",
    #   contact_information: "子育て・若者支援課給付担当,電話：03-5246-1232,ファクス：03-5246-1289"
    # },
    {
      id: 74,
      condition_id: 74,
      support_id: 21,
      city_id: 7,
      payment: 0,
      age_id: 2,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/kosodate/mokutei/teate_josei/teate/jidofuyoteate.html",
      payment_limit: 1,
      payment_frequency: 1,
      payment_month: "なし",
      transfer_destination: "指定した口座",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      contact_information: "子育て・若者支援科（６階⑥窓口）,電話：03-5246-1232"
    },
  ]
)
