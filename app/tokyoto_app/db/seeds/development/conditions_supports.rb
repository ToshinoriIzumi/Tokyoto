ConditionsSupport.create!(
  [
    {
      id: 1,
      condition_id: 1,
      support_id: 7,
      city_id: 7,
      payment: 100000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/iwaikinkofu.html",
      payment_limit: 0,
      payment_frequency: 5,
      payment_month: "記載なし",
      transfer_destination: "記載なし",
      user_application_method: "各幼稚園を通じて、申請",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      application_form: "記載なし",
    },
    ##私立幼稚園等保護者補助金-生活保護-第一子-ある-ある
    {
      id: 2,
      condition_id: 2,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-生活保護-第一子-ある-ない
    {
      id: 3,
      condition_id: 3,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-生活保護-第一子-ない-ある
    {
      id: 4,
      condition_id: 4,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-生活保護-第一子-ない-ない
    {
      id: 5,
      condition_id: 5,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-生活保護-第二子-ある-ある
    {
      id: 6,
      condition_id: 6,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-生活保護-第二子-ある-ない
    {
      id: 7,
      condition_id: 7,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      ser_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-生活保護-第二子-ない-ある
    {
      id: 8,
      condition_id: 8,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-生活保護-第二子-ない-ない
    {
      id: 9,
      condition_id: 9,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
##私立幼稚園等保護者補助金-生活保護-第三子-ある-ある
    {
      id: 10,
      condition_id: 10,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-生活保護-第三子-ある-ない
    {
      id: 11,
      condition_id: 11,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      ser_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-生活保護-第三子-ない-ある
    {
      id: 12,
      condition_id: 12,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                できます。
                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-生活保護-第三子-ない-ない
    {
      id: 13,
      condition_id: 13,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第一子-ある-ある
    {
      id: 14,
      condition_id: 14,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第一子-ある-ない
    {
      id: 15,
      condition_id: 15,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      ser_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第一子-ない-ある
    {
      id: 16,
      condition_id: 16,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第一子-ない-ない
    {
      id: 17,
      condition_id: 17,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第二子-ある-ある
    {
      id: 18,
      condition_id: 18,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第二子-ある-ない
    {
      id: 19,
      condition_id: 19,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      ser_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第二子-ない-ある
    {
      id: 20,
      condition_id: 20,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第二子-ない-ない
    {
      id: 21,
      condition_id: 21,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第三子-ある-ある
    {
      id: 22,
      condition_id: 22,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第三子-ある-ない
    {
      id: 23,
      condition_id: 23,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      ser_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第三子-ない-ある
    {
      id: 24,
      condition_id: 24,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-住民税非課税および住民税所得割非課税-第三子-ない-ない
    {
      id: 25,
      condition_id: 25,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第一子-ある-ある
    {
      id: 26,
      condition_id: 26,
      support_id: 20,
      city_id: 7,
      payment: 141600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第一子-ある-ない
    {
      id: 27,
      condition_id: 27,
      support_id: 20,
      city_id: 7,
      payment: 141600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      ser_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第一子-ない-ある
    {
      id: 28,
      condition_id: 28,
      support_id: 20,
      city_id: 7,
      payment: 141600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第一子-ない-ない
    {
      id: 29,
      condition_id: 29,
      support_id: 20,
      city_id: 7,
      payment: 141600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第二子-ある-ある
    {
      id: 30,
      condition_id: 30,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第二子-ある-ない
    {
      id: 31,
      condition_id: 31,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      ser_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第二子-ない-ある
    {
      id: 32,
      condition_id: 32,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第二子-ない-ない
    {
      id: 33,
      condition_id: 33,
      support_id: 20,
      city_id: 7,
      payment: 177600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第三子-ある-ある
    {
      id: 34,
      condition_id: 34,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第三子-ある-ない
    {
      id: 35,
      condition_id: 35,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      ser_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第三子-ない-ある
    {
      id: 36,
      condition_id: 36,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う
      兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-360万未満-第三子-ない-ない
    {
      id: 37,
      condition_id: 37,
      support_id: 20,
      city_id: 7,
      payment: 193200,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があります。※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※年齢に関わらず、保護者と生計を一にする子のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定する。
      例:区分Ａ～Ｃで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （第１子）
      小学校３年の長女 （第２子）
      幼稚園年長の次女 （第３子）"
    },
    ##私立幼稚園等保護者補助金-680万未満-第一子-ある-ある
    {
      id: 38,
      condition_id: 38,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※小学校１～３年生の子ども及び私立幼稚園等（※１）に通っている園児のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定します。
      例:区分Ｄ～Ｆで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （対象外）
      小学校３年の長女 （第１子）
      幼稚園年長の次女 （第２子）
      幼稚園年中の次男 （第３子）
      ※1兄姉に国立・公立幼稚園、保育園、認定こども園、特別支援学校の幼稚部、児童心理治療施設通所部、児童発達支援、医療型児童発達支援の利用者がいる場合は、下の園児を第２子以降とみなします。"
    },
    ##こっから
    ##私立幼稚園等保護者補助金-680万未満-第一子-ある-ない
    {
      id: 39,
      condition_id: 39,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",

    },
    ##私立幼稚園等保護者補助金-680万未満-第一子-ない-ある
    {
      id: 40,
      condition_id: 40,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                できます。
                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-680万未満-第一子-ない-ない
    {
      id: 41,
      condition_id: 41,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                ③令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-680万未満-第二子-ある-ある
    {
      id: 42,
      condition_id: 42,
      support_id: 20,
      city_id: 7,
      payment: 48000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※小学校１～３年生の子ども及び私立幼稚園等（※１）に通っている園児のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定します。
      例:区分Ｄ～Ｆで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （対象外）
      小学校３年の長女 （第１子）
      幼稚園年長の次女 （第２子）
      幼稚園年中の次男 （第３子）
      ※1兄姉に国立・公立幼稚園、保育園、認定こども園、特別支援学校の幼稚部、児童心理治療施設通所部、児童発達支援、医療型児童発達支援の利用者がいる場合は、下の園児を第２子以降とみなします。"
    },
    ##私立幼稚園等保護者補助金-680万未満-第二子-ある-ない
    {
      id: 43,
      condition_id: 43,
      support_id: 20,
      city_id: 7,
      payment: 48000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                  ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-680万未満-第二子-ない-ある
    {
      id: 44,
      condition_id: 44,
      support_id: 20,
      city_id: 7,
      payment: 48000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                できます。
                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-680万未満-第二子-ない-ない
    {
      id: 45,
      condition_id: 45,
      support_id: 20,
      city_id: 7,
      payment: 48000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                ③令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-680万未満-第三子-ある-ある
    {
      id: 46,
      condition_id: 46,
      support_id: 20,
      city_id: 7,
      payment: 186000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※小学校１～３年生の子ども及び私立幼稚園等（※１）に通っている園児のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定します。
      例:区分Ｄ～Ｆで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （対象外）
      小学校３年の長女 （第１子）
      幼稚園年長の次女 （第２子）
      幼稚園年中の次男 （第３子）
      ※1兄姉に国立・公立幼稚園、保育園、認定こども園、特別支援学校の幼稚部、児童心理治療施設通所部、児童発達支援、医療型児童発達支援の利用者がいる場合は、下の園児を第２子以降とみなします。"
    },
    ##私立幼稚園等保護者補助金-680万未満-第三子-ある-ない
    {
      id: 47,
      condition_id: 47,
      support_id: 20,
      city_id: 7,
      payment: 186000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                  ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-680万未満-第三子-ない-ある
    {
      id: 48,
      condition_id: 48,
      support_id: 20,
      city_id: 7,
      payment: 186000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                できます。
                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-680万未満-第三子-ない-ない
    {
      id: 49,
      condition_id: 49,
      support_id: 20,
      city_id: 7,
      payment: 186000,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                ③令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万未満-第一子-ある-ある
    {
      id: 50,
      condition_id: 50,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※小学校１～３年生の子ども及び私立幼稚園等（※１）に通っている園児のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定します。
      例:区分Ｄ～Ｆで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （対象外）
      小学校３年の長女 （第１子）
      幼稚園年長の次女 （第２子）
      幼稚園年中の次男 （第３子）
      ※1兄姉に国立・公立幼稚園、保育園、認定こども園、特別支援学校の幼稚部、児童心理治療施設通所部、児童発達支援、医療型児童発達支援の利用者がいる場合は、下の園児を第２子以降とみなします。"
    },
    ##私立幼稚園等保護者補助金-730万未満-第一子-ある-ない
    {
      id: 51,
      condition_id: 51,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                  ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万未満-第一子-ない-ある
    {
      id: 52,
      condition_id: 52,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                できます。
                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万未満-第一子-ない-ない
    {
      id: 53,
      condition_id: 53,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                ③令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万未満-第二子-ある-ある
    {
      id: 54,
      condition_id: 54,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請",
      additional_info_of_application: "※ひとり親世帯は申請書にチェックが必要"
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
      additional_info_conditions_supports: "※東京都・台東区の補助金の合計額は、年間保育料の合計額を限度とする
      ※補助金額は、令和４年４月～８月までは令和３年度の区市町村民税所得割額
      ※補助金額決定後に修正申告等で区市町村民税所得割額が変更になった場合は、ご連絡いただければ再度交付決定いた
      します。（補助額が変わらない場合もあります）ただし、現年度内に限る。
      ※年度途中で入園・退園・転入・転出をした場合は、在園期間に応じて補助金額を減額して適用します。
      ※幼稚園等のうち、子ども・子育て支援新制度に移行した園は本補助金の対象ではない(台東初音幼稚園・仰願寺幼稚園など)
      ※小学校１～３年生の子ども及び私立幼稚園等（※１）に通っている園児のうち、一人目を第１子、二人目を第２子、三人目以降を第３子以降として算定します。
      例:区分Ｄ～Ｆで小中学校に通う兄 姉がいる場合
      中学校１年の長男 （対象外）
      小学校３年の長女 （第１子）
      幼稚園年長の次女 （第２子）
      幼稚園年中の次男 （第３子）
      ※1兄姉に国立・公立幼稚園、保育園、認定こども園、特別支援学校の幼稚部、児童心理治療施設通所部、児童発達支援、医療型児童発達支援の利用者がいる場合は、下の園児を第２子以降とみなします。"
    },
    ##私立幼稚園等保護者補助金-730万未満-第二子-ある-ない
    {
      id: 55,
      condition_id: 55,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                  ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万未満-第二子-ない-ある
    {
      id: 56,
      condition_id: 56,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                できます。
                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万未満-第二子-ない-ない
    {
      id: 57,
      condition_id: 57,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                ③令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万未満-第三子-ある-ある
    {
      id: 58,
      condition_id: 58,
      support_id: 20,
      city_id: 7,
      payment: 178800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請, 提出書類: 私立幼稚園等交付申請書兼付申請書兼請求書 ※ひとり親世帯は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万未満-第三子-ある-ない
    {
      id: 59,
      condition_id: 59,
      support_id: 20,
      city_id: 7,
      payment: 178800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                  ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万未満-第三子-ない-ある
    {
      id: 60,
      condition_id: 60,
      support_id: 20,
      city_id: 7,
      payment: 178800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                できます。
                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万未満-第三子-ない-ない
    {
      id: 61,
      condition_id: 61,
      support_id: 20,
      city_id: 7,
      payment: 178800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                ③令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第一子-ある-ある
    {
      id: 62,
      condition_id: 62,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請, 提出書類: 私立幼稚園等交付申請書兼付申請書兼請求書 ※ひとり親世帯は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第一子-ある-ない
    {
      id: 63,
      condition_id: 63,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                  ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第一子-ない-ある
    {
      id: 64,
      condition_id: 64,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                できます。
                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第一子-ない-ない
    {
      id: 65,
      condition_id: 65,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                ③令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第二子-ある-ある
    {
      id: 66,
      condition_id: 66,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請, 提出書類: 私立幼稚園等交付申請書兼付申請書兼請求書 ※ひとり親世帯は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第二子-ある-ない
    {
      id: 67,
      condition_id: 67,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                  ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第二子-ない-ある
    {
      id: 68,
      condition_id: 68,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                できます。
                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第二子-ない-ない
    {
      id: 69,
      condition_id: 69,
      support_id: 20,
      city_id: 7,
      payment: 21600,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                ③令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第三子-ある-ある
    {
      id: 70,
      condition_id: 70,
      support_id: 20,
      city_id: 7,
      payment: 118800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請, 提出書類: 私立幼稚園等交付申請書兼付申請書兼請求書 ※ひとり親世帯は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第三子-ある-ない
    {
      id: 71,
      condition_id: 71,
      support_id: 20,
      city_id: 7,
      payment: 118800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                  ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第三子-ない-ある
    {
      id: 72,
      condition_id: 72,
      support_id: 20,
      city_id: 7,
      payment: 118800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                できます。
                                ※提出書類②は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
    ##私立幼稚園等保護者補助金-730万以上-第三子-ない-ない
    {
      id: 73,
      condition_id: 73,
      support_id: 20,
      city_id: 7,
      payment: 118800,
      age_id: 1,
      url: "https://www.city.taito.lg.jp/kosodatekyouiku/yochien/shiritsuyochien/hogoshahojo.html",
      payment_limit: 1,
      payment_frequency: 3,
      payment_month: "9月中旬(4~6月分),12月中旬(7~9月分),翌年3月(10-12月分),翌年6月中旬(1-3月)",
      transfer_destination: "指定した口座",
      user_application_method: "各幼稚園を通じて申請書等をお配りし(5月中旬~6月上旬)、各幼稚園を通じて申請
                                提出書類
                                ①私立幼稚園等交付申請書兼付申請書兼請求書
                                ②令和3年度区市町村民税（非課税）証明書（原本又はコピー）又は令和3年度納税（決定）通知書（原本又はコピー）
                                ③令和4年度区市町村民税（非課税）証明書（原本又はコピー）又は令和4年度納税（決定）通知書（原本又はコピー）
                                ・課税（非課税）証明書は、住民登録のあった区市町村で取得できます。

                                ※提出書類②・③は、同居・別居の如何を問わず、園児と生計を一にする方全員分を提出する必要があり
                                ※ 指定都市で課税されている方は、旧税率（6％）にて算出した市民税所得割額を用いて補助金額を決定します。
                                ひとり親は申請書にチェックが必要",
      user_application_limit: "記載なし",
      user_renewal_method: "記載なし",
      user_renewal_month: "記載なし",
    },
  ]
)
